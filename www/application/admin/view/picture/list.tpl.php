...
<div class="layui-tab layui-tab-brief main-tab-container">
    <ul class="layui-tab-title main-tab-title">
      <a href="<?php echo url('picture/add','category_id='.$category_id) ?>"><li>添加图片</li></a>
        ...
        <table class="list-table">
          ...
          <?php foreach ($pictures as $v) { ?>
            <tr>
              ...
              <td><?php echo $v['id'] ?></td>
              <td>
                <a class="list-title" href="<?php echo url('index/picture/show','id='.$v['id']) ?>" target="_blank"><?php echo $v['title']; ?></a>
                <?php if($v['image_url']){ ?>
              ...
              <td style="text-align: center;">
              <a href="<?php echo url('picture/edit','id='.$v['id']) ?>" class="layui-btn layui-btn-small" title="编辑"><i class="layui-icon"></i></a>
              <a class="layui-btn layui-btn-small layui-btn-danger del_btn" picture-id="<?php echo $v['id'] ?>" title="删除" picture-name='<?php echo $v['title'] ?>'><i class="layui-icon"></i></a>
              </td>
            </tr>
          <?php } ?>
          </tbody>
...
layui.use(['element', 'laypage', 'layer', 'form'], function(){
  ...
  jq('.is_recommend').click(function(){
    var id = jq(this).attr('data-id');
      ...
      var row = jq(this);
      jq.post('{:url("picture/to_recommend")}',{'id':id},function(data){
        if(data.code == 200){
  ...
  jq('.is_top').click(function(){
    var id = jq(this).attr('data-id');
      ...
      var row = jq(this);
      jq.post('{:url("picture/to_top")}',{'id':id},function(data){
        if(data.code == 200){
          if(row.hasClass('list-switch-off')){
            row.removeClass('list-switch-off').find('.layui-icon').html('&#xe605;');
            row.attr('title','点击开启');
  ...
  jq('.del_btn').click(function(){
    var name = jq(this).attr('picture-name');
    var id = jq(this).attr('picture-id');
    layer.confirm('确定删除【'+name+'】?', function(index){
      loading = layer.load(2, {
        shade: [0.2,'#000'] //0.2透明度的白色背景
      });
      jq.post('{:url("picture/del")}',{'id':id},function(data){
        if(data.code == 200){
          layer.close(loading);
  ...
  form.on('submit(delete)', function(data){
    ...
    layer.confirm('确定批量删除?', function(index){
      ...
      jq.post('{:url("picture/batches_delete")}',param,function(data){
        if(data.code == 200){
          layer.close(loading);
          layer.msg(data.msg, {icon: 1, time: 1000}, function(){
            location.reload();//do something
    ...
    jq.post('{:url("category/model_category_select")}',param,function(data){
      ...
      form.on('select(move)', function(data){
        ...
        layer.confirm('确定批量移动至【'+selected_text+'】?', function(index){
          ...
          jq.post('{:url("picture/batches_move")}',param,function(data){
            if(data.code == 200){
              layer.close(loading);
              layer.msg(data.msg, {icon: 1, time: 1000}, function(){
                location.reload();//do something
